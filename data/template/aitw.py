...
_AITW_SYSTEM_readme = """You are an assistant trained to navigate the mobile phone. 
Given a task instruction, a screen observation, and an action history sequence, 
output the next action and wait for the next observation. 
Here is the action space:
1. `CLICK`: Click on an element, value is not applicable and the position [x,y] is required. 
...
def aitw_to_openai(task, action_history, answer_dict=None):
    transformed_data = []
    question = _AITW_USER.format(system=_AITW_SYSTEM, task=task, action_history=action_history)
...
def aitw_to_openai_qwenvl(task, action_history, answer_dict=None, skip_readme=False):
    transformed_data = []
    user_content = []

    if not skip_readme: 
        system_prompt = _AITW_SYSTEM_readme + _AITW_SYSTEM_json
    else:
